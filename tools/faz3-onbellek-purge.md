...
Mühendis notu (16 Tem 2026 · güncel main üzerine yeniden uygulandı 20 Tem 2026). Kararı
mimar/Okan verir; burada **ne yapıldığı + neden** ve **açık bırakılan seçim** yazılı.
Kod yorumu değil buraya yazıldı: bu bir işletme kararı (tazelik ↔ maliyet), kodun detayı değil.

> **20 Tem durumu:** Worker'da `/katalog` ucu **HENÜZ CANLI DEĞİL** (ayrı repo
> `~/dev/pruvo-bot`, main'de yok; canlı Worker `/katalog`'a 403 dönüyor, `/ara` 200).
> Aşağıdaki `/katalog` satırları **hedef sözleşmedir**, ölçülmüş canlı durum değil.
> `/ara` satırları FAZ 1'den beri canlı ve ölçülü.
