...
<script>
  ...
  /* 🔴 RIZA ALAN KUMESI — TEK KANONIK KAYNAK. Riza bandinin DORT ayri script varyanti
     (uretec + dort kaynak sayfa) bu kumeyi ELLE TEKRARLAMAZ; yalniz pruvoRizaUygula()'yi
     cagirir. VARSAYILAN yukarida HEPSI 'denied'; burasi yalnizca "onay gelirse ne acilir"
     tanimidir — gevsetme YOKTUR. */
  ...
  /* Ziyaretçi daha önce onay verdiyse geri yükle.
     ⚠️ ESKİ (DAR) RIZA KAYDI: reklam çerezinden HİÇ söz etmeyen bir metinle alınmış onay,
     reklam alanlarına SAYILMAZ — yalnız analitik açılır, ad_* 'denied' KALIR ve bant
     genişletilmiş kapsam için bir kez daha sorar. Sessiz genişletme açık rıza değildir. */
  try {
    if (localStorage.getItem('pruvo_onay_analitik') === 'kabul') {
...
(function(){
  ...
  /* Kapsam adı TEK KANONİK KAYNAKTAN (GA head bloğu) gelir. Blok bir sebeple
     çalışmadıysa "" döner → kapsam kaydı YAZILMAZ ve bant sormaya devam eder
     (fail-closed; belirsizlik rıza değildir). İkinci bir sabit TANIMLANMAZ. */
  ...
  try {
    secim = localStorage.getItem(ANAHTAR);
    kapsam = localStorage.getItem(KAPSAM_ANAHTARI);
  } catch(e){}
  /* 🔴 KAPSAM GENİŞLEDİ (reklam çerezi eklendi): DAR kapsamda verilmiş eski onay yeni
     kapsama SAYILMAZ → bant bir kez daha sorar. "Reddet" diyene TEKRAR SORULMAZ. */
  if(secim === "ret"){ return; }
  ...
  function kaydet(deger){
    ...
    /* "Reddet" = daha önce verilmiş onay varsa GERİ ÇEKME: dört alan da denied. */
    if(deger !== "kabul" && typeof window.pruvoRizaUygula === "function"){
      window.pruvoRizaUygula('denied');
    }
    el.hidden = true;
  }
