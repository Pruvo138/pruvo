    ...
    for t in hedefler:
        if _rakam(t) != wa:
            fails.append("%s: gömülü ref-atıf betiğinin TARGET_PHONE'u (%s) "
                         ...
                         % (ad, t, wa))
    return (len(bulunan), len(dinamik), len(kisa))
...
def belge_denetle(ad, doc, fails, taban=None, sessiz=False):
    ...
    literal, dinamik, kisa = numara_kontrol(ad, doc, fails, taban=taban, sessiz=sessiz)
...
def fikstur_kontrol(fails):
    ...
    _mutasyon("İLGİSİZ (A22): basılan numaranın tipografisi", gorunur_s,
              ...
              fails, False)
    # CTA varlık nöbetinin YANLIŞ-POZİTİF ekseni: bu üçü YEŞİL kalmalı
    _mutasyon_re("İLGİSİZ: href'siz (JS dolduran) WhatsApp butonu eklemek", idx,
                 r"</body>",
                 lambda m: '<a class="x" target="_blank">WhatsApp ile Sipariş</a>\n'
                           + m.group(0), fails, False)
    _mutasyon_re("İLGİSİZ: WhatsApp'tan söz eden (linksiz) paragraf", icerik,
                 r"<p>", lambda m: "<p>WhatsApp üzerinden de yazabilirsiniz. ",
                 fails, False)
    _mutasyon_re("İLGİSİZ: CTA butonunun etiket metnini değiştirmek", idx,
                 r"WhatsApp'tan Yaz", lambda m: "WhatsApp'tan Sor", fails, False)
...
KÖR NOKTALAR (bu nöbetçinin ÖLÇMEDİĞİ şeyler — sessiz kalmasın diye SAYIYLA yazılır):
  ...
  3. 🔴 CANLI KAÇAK (beyan): görünür metinde NÖTR bağlamdaki telefon literalinin HANGİ
     ...
     üretirdi. ŞABLON DAĞILIMI (bağımsız ölçümle düzeltildi; önce yanlışlıkla "hepsi
     tek şablon" yazılmıştı): 13 literalin bağlam metni 13'ü de FARKLI, yalnız 11'inde
     "Sipariş" başlığı geçiyor; kalan 2'si shop/parametrik "Aralık dışı..." cümlesinden.
     Hepsi bugün WhatsApp numarasını taşıyor (ihlal YOK).
