...
Once tools/build.py'yi CALISTIRIR (uretilen sayfalar taze olsun), sonra sirayla:
  1. urunler.json bu kosumda DEGISMEDI (sha256 once/sonra esit — paket urun verisine
     YAZMAZ); 17 Agu 2026: eski eksen "git diff --name-only -- urunler.json" BOS olsun idi;
     bu checkout 5 ev paylasiyor, komşunun meşru yarim isi dosyayi kirletir -> TEST 1 komsunun
     durumunu olcuyordu (16 Agu'da olculdu: 25/26 yesil, TEST 1 komsunun kirli urunler.json'i
     yuzunden kirmizi). Yeni eksen "bu kosum dosyayi DEGISTIRDI MI?" — paylasilan agacta
     komsunun kirli ağaci kapıyı YAKMAZ ve iddia GUCELENIR.
...
def main():
    # ---- 1'in on-eksi (SHA256) burada ALINIR, karsilastirmasi TEST 1'de EN SONDA yapilir.
    # Eski eksen "git diff --name-only -- urunler.json" BOS olsun idi; bu checkout 5 ev
    # paylasiyor, komşunun meşru yarim isi dosyayi kirletiyor ve kapı kırmızı yanıyordu
    # (paylasilan agacta komsunun durumu, iddia olmasi gereken sey degil — olculdu 16 Agu).
    # Yeni eksen: BU KOSUM urunler.json'a YAZDI MI? once/sonra sha256 esitligi.
    with open(os.path.join(ROOT, "urunler.json"), "rb") as f:
        URUNLER_JSON_SHA_ONCE = hashlib.sha256(f.read()).hexdigest()
    ...
    try:
        ...
        # ---- 1) urunler.json bu kosumda DEGISMEMIS — sha256 once/sonra esit
        # Eski eksen "git diff --name-only -- urunler.json" BOS olsun idi; paylasilan checkout'ta
        # komsunun kirli agaci kapıyı YAKIYORDU (TEST 1 = komsunun durumu, paketin iddiasi degil).
        # Karsilastirma main()'in SONUNDA yapilir (URUNLER_JSON_SHA_ONCE ile AFTER).
        ...
        if kaldi:
            print("SONUC (ara): %d/%d test KIRMIZI ❌ (TEST 1 henuz eklenmedi, finally'ye bakin)"
                  % (len(kaldi), len(SONUC)))
            sys.exit(1)
        print("SONUC (ara): %d/%d test YESIL ✅ (TEST 1 henuz eklenmedi, finally'ye bakin)"
              % (len(SONUC), len(SONUC)))



    finally:
        # ---- 1) urunler.json bu kosumda DEGISMEDI (sha256 once/sonra esit)
        # Eski eksen ('git diff --name-only -- urunler.json' BOS olsun)
        # paylasilan checkout'ta komsunun kirli agaci kapıyı YAKIYORDU — iddia
        # olmasi gereken sey degil. Yeni eksen: bu kosum dosyayi DEGISTIRDI MI?
        # on-eksi SHA256 main() basinda alindi; karsilastirma finally'de (paketin
        # tum testleri bittikten SONRA + erken crash / sys.exit yollarinda bile) yapilir
        # -> komsunun kirli ağaci kapıyı yakmaz ve iddia GUCELENIR.
        try:
            with open(os.path.join(ROOT, 'urunler.json'), 'rb') as f:
                _sonra = hashlib.sha256(f.read()).hexdigest()
        except Exception as _ex:
            kayit(1, 'urunler.json bu kosumda DEGISMEDI (sha256 once/sonra esit)',
                  False, 'sha256 okunamadi: %r' % _ex)
        else:
            _degisti = _sonra != URUNLER_JSON_SHA_ONCE
            kayit(1, 'urunler.json bu kosumda DEGISMEDI (sha256 once/sonra esit — paket'
                  ' urun verisine YAZMAZ; paylasilan agacta komsunun kirli durumu'
                  ' kapıyı YAKMAZ)',
                  not _degisti,
                  ('degisti: %s... -> %s...' % (URUNLER_JSON_SHA_ONCE[:12], _sonra[:12]))
                  if _degisti else 'temiz (%s... esit)' % URUNLER_JSON_SHA_ONCE[:12])

        # Nihai ozet (finally icinde — TEST 1 dahil)
        print("-" * 70)
        _kaldi = [x for x in SONUC if not x[2]]
        if _kaldi:
            print("SONUC: %d/%d test KIRMIZI ❌" % (len(_kaldi), len(SONUC)))
            sys.exit(1)
