...
# 🔴 W2 (16 Eyl 2026): env SET EDILMEZSE davranis BIREBIR eskisi; yalniz izole
# sandbox/CI kosumu mumkun olur (bkz. nobet-kapi.py ayni sabit).
...
MUTANTLAR = [
    ...
    # 🔴 K350 BAYAT OKUYUCU MUTANT (29 Agu 2026, cip KraL-K350-TeslimTatbikati-29Agu):
    # bayat okuyucu onarimi `llm_turu = False` ile susuyordu; burada `bool(...)`
    # capraz kablo cekilirse eski `True` degeri YASAR ve satir yine `LLM_TURU=1`
    # basar. Onarimin KENDI davranissal ekseni budur; M17 olmadan bu kol
    # sadece "yazildi, var" diye yesil gorunur.
    ("M17 llm_turu bayat okuyucuya doner (kol KAPALIYKEN 1 basar)",
     GOZCU_YOLU, "tur", r"^\s+llm_turu = False\s*$",
     _yerine("llm_turu = False", "llm_turu = bool(llm_turu)")),
