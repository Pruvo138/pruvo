...
"""KABUL — ANASAYFA CIPI <-> /marka/<slug>/ SAYFASI: gorunen her cipin sayfasi VAR ve DOLU.
  ...
  python3 tools/cip-sayfa-test.py              # kabul (CI'da bloklayici)
  python3 tools/cip-sayfa-test.py --mutasyon   # cift yonlu mutasyon (elle)
  python3 tools/cip-sayfa-test.py --kok /yol   # BASKA agactan oku (mutasyon icin)
...
def mutasyon():
    ...
    for i, (dosya, eski, yeni, beklenen, aciklama) in enumerate(MUTANTLAR, 1):
        ...
        try:
            ...
            p = subprocess.run([sys.executable, os.path.join(tmp, "tools", "cip-sayfa-test.py"),
