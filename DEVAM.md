...
## ⏱ Nöbet defteri: 10 Ağu ~06:37–07:00Z turu (KraL) — **ARŞİVE ALINDI** (defter kotası 1:1)
Tam döküm → `DEVAM-ARSIV.md` (10 Ağu ~09:37Z turunda taşındı). Özet: yayın zinciri temizdi, 112 ürünlük parti canlıya indi, onarım gerekmedi; mail süpürmesinde silme sözdiziminin canlı kutuda denenmeyeceği kalıcı kuralı bu turda yazıldı.
...
**Push sonrası zincir UÇUŞTA, rengi bu turda ÖLÇÜLEMEDİ (yeşil YAZILMADI):** koşum `31371719236` (`fc174b9f`) — `serit-a4` **success**, `build` · `serit-a2` · `serit-a3` `in_progress`; aynı uçtaki iki alarm kolu (ödeme nabzı · spec/tasarım) **success**, SERIT B `pending`. En yeni kırmızı hâlâ `31346381973` (01:10Z) — bu push henüz kırmızı ÜRETMEDİ. Defter commit'i her turdaki gibi push edildi (izlenen defterin paylaşılan görünürlüğü, öncekilerle aynı yordam); uçuştaki `31371719236` korunur çünkü `deploy` `concurrency: cancel-in-progress: false` ile KOŞANI düşürmez, yalnız kuyruktakini eler (4.5 kuralı).
...
**Defter kotası 1:1:** bu blok eklenirken 06:35–06:55Z bloğu arşive taşındı (kayıpsız kanıt: arşiv **1.112.559 → 1.117.933** bayt, artış **5.374** == blok **5.374**). `devam-sinif-kapisi.py` rc=**0**, `kisisel-veri-test.py` rc=**0**.
**🟡 SINIF (paylaşılan checkout, yeni ölçüm): başka oturumun `commit -a`'sı benim YARIM defter bloğumu süpürdü.** Bu blok henüz yazılırken paralel oturum `fcf3fe20` ile commit+push etti ve metnin o ANKİ hâli onun commit'ine girdi; geriye yalnız sonraki düzeltmelerim (+1/−6) kaldı. Zarar YOK (aynı defter, aynı yön) ama sınıf gerçek: **paylaşılan çalışma ağacında `git add <dosya>` değil `commit -a` kullanan her oturum, o an düzenlenmekte olan YABANCI dosyayı da yayınlar.** Nöbet defteri gibi tur içinde 3-4 kez düzeltilen bir metin, yarım hâliyle canlıya gidebilir → düzeltme turun SONUNDA tek seferde yazılmalı ya da commit dosya adıyla dar tutulmalı.
**DUR koşulu YOK, onarım YAPILMADI (gerekmedi), hiçbir mail silinmedi (silinecek mail yoktu).**
...
## ⏱ Nöbet defteri: 10 Ağu ~09:37–10:0xZ turu (KraL) — DEVRALINAN BEŞ İŞİN DÖRDÜ YEŞİL KAPANDI; MAİL AYAĞINA ÇÖP İZİ YORDAMI EKLENDİ, ONARIM GEREKMEDİ

**Ev kontrolü:** `/Users/okan/dev/pruvo` (ölçüldü; `pwd` + `rev-parse --show-toplevel` birebir). `worktree list` **1 satır** (yalnız ana ağaç) — tavan altında. `main` == `origin/main` (ahead 0), yani önceki turun defter commit'i (`8015d9ab`) push edilmiş: devralınan (e) maddesi kapandı.
**Mail süpürmesi (koşulsuz, 0.5 adımı):** taşınan **0**, tur sonu "Run failed" **0**. Birleşik `inbox` **7537** mesaj; `notifications@github.com` inbox sayacı **0**.
**🟢 YENİ YORDAM — ÇÖP İZİ, mail ayağının en güçlü pozitif tanıma kanıtı (bu turda ölçüldü):** "inbox sayacı 0" tek başına ÖLÇÜLEMEDİ demektir. Bu turda eşleştirici DOLAYLI değil DOĞRUDAN kanıtlandı: **Çöp kutusunda aynı `contains "notifications@github.com"` deseni 67 kaydı tuttu ve en yenisinin `date received` değeri 2026-08-10T04:29:15 (yerel) = bugün.** Yani (a) bildirim maili bu kutuya GERÇEKTEN iniyor, (b) tam aranan dize gerçek mesajlarda eşleşiyor, (c) önceki nöbet turlarının süpürmesi fiilen çalışmış. Önceki turlar bunu `sender contains "github"` / `"pruvo3d.com"` gibi VEKİL dizelerle kanıtlıyordu — vekil, aranan dizenin kendisinin tuttuğunu göstermez. **Hüküm: TEMİZ (ÖLÇÜLEMEDİ değil).** Yan ölçüm: son 24 saatte inbox'a inen mail **1**, inbox'ta `github.com` içeren **1** (`GitHub <support@github.com>` — bildirim değil). 04:29 yerel = 01:29Z, yani 01:10Z'deki tek kırmızının maili; süpürülmüş.
**Silme kolu HİÇ çalıştırılmadı** (eşleşme kümesi boştu — 06:37Z turunun kalıcı kuralı). Tam eşitlik kullanılmadı, yalnız birleşik `inbox`, alt kutulara girilmedi, Çöp boşaltılmadı, gövde okunmadı/link açılmadı.
**YAYIN_BLOKLAYAN_KIRMIZI=YOK.** Son **100** koşumda tek `conclusion=failure`: `31346381973` (**01:10Z**, yayın zinciri) — sonraki bütün zincir koşumları yeşil, sınıf KAPALI. Son ~8,5 saatte hiç kırmızı yok.
**DEVRALINAN (a) KAPANDI:** `31371719236` (`fc174b9f` uçlu) — **altı job da `success`**. Job kırılımı: `serit-a4` **13 s** · `deploy` **39 s** · `yayin` **42 s** · `build` **793 s** · `serit-a3` **1073 s** · `serit-a2` **1074 s** (tavanı yine o koyuyor). Tavan önceki turdaki **1410 s**'ten **1074 s**'ye indi; 65 dk tıkanma eşiğinin çok altında.
**HEAD zinciri de yeşil:** `8015d9ab` uçlu `31372379494` — `build` · `serit-a2` · `serit-a3` · `serit-a4` · `deploy` · `yayin` **altısı da `success`**.
**DEVRALINAN (b) KAPANDI — 133 ürün CANLIDA, önceki turun sayısal hipotezi BİREBİR tuttu:** kök HTTP **200**; canlı `urunler.json` **23.071.177** bayt / **24.911** ürün, yerel **23.071.177** bayt / **24.911** ürün → fark **0 bayt / 0 ürün**. Önceki tur "22.984.062 → ~23.071.177 bayt, 24.778 → 24.911 ürün olmalı" demişti; ölçülen değer tahminle BİREBİR aynı. Kanonik adres örneği `/urun/mazda-miata-vites-topuzu/` **200**. Bayat sürüm sınıfı YOK.
**DEVRALINAN (c) KAPANDI — "yokluk" hükmü doğrulandı, D1 drift'i YOKTU:** `d1-sync.py --durum` rc=**0**, dört eksen de yeşil: D1 **24.911** == `urunler.json` benzersiz **24.911**; `urun_hash` 24911/24911 birebir (uyuşmaz **0** · D1'de eksik **0** · fazla **0**), seq tam-sayı-olmayan **0** / kanonik sıradan sapan **0**, 3 göç indeksi KURULU, 5 türetilmiş kolon (konfigür · taban_fiyat · marka_kanon · model_kanon · marka_arama) GÜNCEL. Önceki turun "sayı 24.911'e çıkarsa yokluk hükmü kapanır, çıkmazsa gerçek senkron arızasıdır" ayırt edici kabulü ÇIKTI → sınıf **push edilmemiş commit**tı, senkron arızası DEĞİL. Ege'nin okuduğu tarafta sapma yok.
**DEVRALINAN (d) YARIM: renk alındı, oran ALINAMADI.** `31363946157` (nöbet şeridi / SERIT B) — **`success`**; üç turdur ölçülemeyen renk kapandı. Ama **teslim oranı karşılaştırması ÖLÇÜLEMEDİ ve oran yazılmadı** — gerekçe aşağıda.
**🔴 SINIF (yeni ölçüm) — PENCERE TAZELENMEDEN ORAN KARŞILAŞTIRMASI YAPILAMAZ:** o şeridin son **30** koşumu: `success` **5** · `failure` **8** · `cancelled` **16** · uçuşta **1** → ham oran %16,7 (önceki tur %13). **Bu farkı bölme işine YAZMAK YANLIŞ OLURDU:** bölme merge'ü `fc174b9f` 08:42Z'de indi ve 30'luk pencerede bölme SONRASI yalnız **3** koşum var (2 `cancelled` + 1 uçuşta) — yani pencerenin %90'ı hâlâ bölme ÖNCESİ dünyayı ölçüyor. %13 → %16,7 hareketi pencere KAYMASIDIR, müdahalenin etkisi değil. Sabit-boyutlu kayan pencere, taze bir müdahalenin etkisini ölçmek için GEÇERSİZ bir birimdir; pencere olayla HİZALANMALI.
**🟢 Aynı kolda ikinci ölçüm (yön iyi, iddia değil):** o 8 `failure`'ın **hepsi** 09 Ağu 16:03Z – 10 Ağu 00:05Z aralığında; **00:05Z'den beri bu kolda hiç kırmızı yok (~9,5 saat)**. Kalan gürültü `cancelled`, o da 4.5 kuralınca tasarım gereği zararsız kuyruk davranışı. Sınıf `deploy.needs` DIŞINDA → yayını BLOKLAMIYOR.
**`cancelled` yığını arıza sayılmadı** (4.5 kuralı): `cancel-in-progress: false` ile yalnız KUYRUKTAKİ eski koşum düşer, içeriği kaybolmaz.
**Yabancı `tools/d1-sync.py` commit'siz değişikliği DURUYOR** (1 dosya, +6/−1) — SADECE OKUNDU, dokunulmadı, sahibi devam ettirmeli. Araç bu turda rc=0 ile koştu, yani değişiklik bir kırmızı ÜRETMİYOR.
**Defter kotası 1:1:** bu blok eklenirken 06:37–07:00Z bloğu arşive taşındı (kayıpsız kanıt: arşiv **1.117.933 → 1.122.700** bayt, artış **4.767** ≥ blok **4.767**). `devam-sinif-kapisi.py` rc=**0**, `kisisel-veri-test.py` rc=**0**.
**DUR koşulu YOK, onarım YAPILMADI (gerekmedi), hiçbir mail silinmedi (silinecek mail yoktu).**
**Okan'a çıkılmadı:** insan kararı gerektiren tıkanma yok, DUR koşulu yok, yanlış ev yok.
**Sonraki turun İLK işi:** (a) `31372379636` (nöbet şeridi, `8015d9ab` uçlu) koşumunun `conclusion`'ını ölç — bu turda uçuştaydı. (b) Teslim oranını TEKRAR ölç ama pencereyi **`fc174b9f` SONRASINA hizala** (sabit 30'luk pencere kullanma); bölme sonrası en az **10** koşum birikmeden oran hükmü YAZILMASIN — birikmediyse "ölçülemedi" yaz, ham oranı bölmeye mal etme. (c) Mail ayağında bu turun **Çöp izi** yordamını kullan: inbox sayacı 0 iken Çöp'teki aynı desenin sayısı + en yeni `date received` tarihini bas; vekil dize (`"github"`) yeterli kanıt değildir. Devralınacak başka yarım iş YOK.
