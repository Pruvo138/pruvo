...
- **K388 META ATIF — İKİ UÇTA DA `fbc`=0, ÇAPRAZ DOĞRULANDI (TAM METIN KUTUDA):** D1 `siparisler` son 30 gun (varlik sayimi): toplam **33** · atif 27 · `fbp` **19** · **`fbc` 0** · `ga_client_id` 20 · **`utm` 0**; `json_each(atif)` anahtarlari SADECE `ga_client_id`/`fbp`/`ref`. ArTisT'in Meta paneli AYNI SIFIRI verdi (26 Purchase'ta `fbc` %0) → **boru fbc'yi DUSURMUYOR, dusecek fbc YOK.** `fbp`=19 oldugu icin "kok neden riza kapisi" onculu CURUDU; ayiran olcum `index.html:1858` — utm yakalama riza kapisinin ARKASINDA DEGIL. 🔴 TUZAK: `reklam_ref_gclid.created_at` **epoch-ms INTEGER**; ISO filtresi `0` bastı (SAHTE YESIL), dogru kolla **son 30 gunde 162 reklam inisi** (gclid 55 · gbraid 22, hepsi GOOGLE). Yani trafik VAR, 33 siparisin hicbirine baglanmadi. `external_id` kodda **0 atif** (PII degil, ayri kalem; atfi DUZELTMEZ). OLCULEMEDI: Meta inisini rizadan bagimsiz sayan sayac YOK → ArTisT'e tek soru: 8 kampanyanin 28 gunluk Link Tiklamasi/Acilis Goruntuleme sayisi (≈0 ise kalem pazarlamada ve KAPANIR; yuzlerce ise riza kapisi tum Meta atfini kesiyor → Okan/BaBa).
