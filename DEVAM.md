...
- 🔴🔴 **K119 (16 Agu, EN YUKSEK ONCELIK) — YAYIN 15 AGU 14:02'DEN BERI KAPALIYDI.**
  ...
  **Engeller sirayla kaldirildi:**
  1. `serit-a3` KIRMIZI — `.urun-kaynaklari.json`'da 1 tedarikci adi + 3 taninmayan
     e-posta alan adi (iki urun partisinden). Temizlendi → `62386a64`; 4 hata → **0**,
     `denetim-kapisi` IHLAL=0. (31 kayit: 28'inden `uyelik`, 3'unden e-postali tasarimci
     alani notrlestirildi.)
  ...
  🔴 **IKI BYPASS OLDU (isci karari, kayit):** `PRUVO_MUKERRER_ATLA=1` (K119) ve
  `.mukerrer-istisna.json`'un worktree'ye elle kopyalanmasi (K119-B). Ikisi de mukerrer
  kancasini atlatti; sebep `onarim-commit.py`'nin worktree kurarken gitignore'lu istisna
  dosyasini TASIMAMASI. Ayri kalem, sinif kapisi gerekiyor.
- 🔐 **K120 (OKAN KARARI, 16 Agu — UYGULANMADI, yayin acilinca yapilacak):**
  `.urun-kaynaklari.json` **git'ten CIKACAK** (gitignore), yalniz diskte yasayacak;
  silinen `uyelik`/kaynak alanlari geri yazilacak. Gerekce: CLAUDE.md dosyayi "gizli"
  sayiyor ama dosya **IZLENEN** ve repo PUBLIC — doktrin ile kapi celisiyordu, Okan
  celiskiyi dosyayi izlemeden cikararak cozdu.
  🔴 **UYARI (uygulayacak tura):** dosya CI checkout'unda ARTIK OLMAYACAK; onu okuyan
  kapilar (`kisisel-veri-test`, `denetim-kapisi`, hasat araclari) yoklukta **fail-open'a
  DUSMEMELI** — "dosya yok = yesil" bir sessiz delik olur. Her kapinin yokluk kolu
  olculmeden bu is kapanmaz. Ayrica ad GECMISTE zaten public oldu; gitignore geriye
  donuk temizlik DEGILDIR.
- 🔧 **K118 (YENI, 16 Agu — MaCiT bildirdi, HUKUM BENDE):** pre-push sizinti kapisi
  ...
  isciler `--no-verify`'a suruklendi (bir kez OLDU: `d8c48ad9`).
