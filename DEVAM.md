...
## ✅ 6 EYL — `KraL-BorcKapisi-6Eyl` [Opus 5]: **T4: OKAN-KAPISI kilidi KALKTI + sessiz atlama KOL OLDU** (BaBa 16:5x)
**(a)** T4-BORC sayimindan **OKAN-KAPISI CIKTI** (`BORC_DURUMLARI`=ACIK/UCUSTA/🔧). PARSER sozlesmesi `ACIK_DURUMLAR` DEGISMEDI — onu 4 okuyucu daha okuyor (devir-kapisi · korgoz x2 · parti-kapisi), daraltmak olcumlerini oynatirdi. Baski KALDI: `OKAN_KAPISI=N` satiri + 23:00 ozetinde ayri satir. **FaR `acik_kalem=5` → `ACIK_KALEM=2` + `OKAN_KAPISI=3` (F21,F46,F62).** Olcut kolu (K289) da BORCLU'ya daraltildi — yoksa `kabul:`siz tek OKAN-KAPISI satiri evi gene kilitler, (a) INERT kalirdi.
**(b)** Durum hucresi kanonik BES degerin disindaysa satir **ATLANMIYOR**: `T4-OLCULEMEDI durum gecersiz <kimlik>=<metin>` fail-closed RED. F08 kalibi (`ACIK (Okan kapisi KALKTI)`) iki kumeye de girmedigi icin **SESSIZCE ATLANIYORDU** = yanlis-yesil. 🔴 **TUZAK OLCULDU:** markdown AYRAC satiri 8 kolon uretip `kolonlar[5]="---"` verir — elenmeseydi HER defter aninda kirmizi yanardi (kapinin kendi ayracina takilmasi).
**FILO ON-OLCUMU (8 ev, ONCE):** gecersiz hucre **HocA 6 · KraL 7 · ORTAK 7 · digeri 0**. HocA'nin `OKAN-KAPISI (dusuk oncelik)` satiri **bugun sessizce sayilmiyor** → HocA `GECER`→`OLCULEMEDI` (KraL/ORTAK zaten RED, kol degisti). 🔴 Ev kendi defterini KENDI duzeltir.
**KABUL:** `--kendini-test` **8/8 + KONTROL 1/1** (M-a/M-b IZOLE KOPYADA: benzersiz ad + `dont_write_bytecode`; canli govdeye yama YOK, gercek ev yoluna silme YOK) · `--curutme-test` **4/4** · **OLDURUCU 3/3** (kolu geri alan 3 yama bataryayi ADIYLA kirmizi yakti) · 23:00 ozet kolu **3/3** (T4 yok/cokuyor → satir DUSMEZ, `ÖLÇÜLEMEDİ`).
**TABAN URETILDI:** `parti-kapisi` **12/13** + `korgoz_kabul` **KX=4/7** HEAD'de de AYNI (gecici kopya) → regresyon **0**. Yesil: `ev-haritasi` 9/9+4/4 · `chip-duzeni` 14/14+9/9 · `devir-kapisi` 5/5+4/4 · `nobet-kabul` 51/0 · `is-akisi` ✅ · `ci-kapsam` ✅. `nobet.yml` adim adi bayatti (**"4 mutant"**→8+1+4); kablo KORUNDU.
