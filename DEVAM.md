...
**LISANS:** 243 kayit duzeltildi (237 SA + 6 ND), ATIF degisen 0, NC riski 0. Merge `fdccc54d`. TAM METIN KUTUDA (8 Eyl kapanis blogu).
**K19 YAYIN ARIZASI:** `Saab|900` ALLOW · `Yamaha|900` DENY (ciplak hacim sayisi, emsal `Yamaha|660`); uc imza civilendi, kapi 1/29 KALDI = 29/29 GECTI. Merge `d0d6b8f9`. TAM METIN KUTUDA.
**K382 KAPANDI:** yedek karantinasi mesru rotasyonu dusus saniyordu; iki ad `surekli` sinifina alindi, ARDISIK 4 = 0. Merge `ede460cc`. TAM METIN KUTUDA.
...
## 🔴 K388 — META ATIF (9 Eyl, ArTisT ihbari; TAM METIN KUTUDA)
ArTisT: 30 gunde 26 Purchase (tarayici 14/CAPI 16), Eslestirme Kalitesi **4,4/10**, 8 kampanyada Satin Alma **0**. KOD OLCULDU (`shop/src/olcum.js`): 20 Tem'in 3 kaleminden **(1) yanit loglama ve (2) IP/UA KAPALI** — ikisi de uygulanmis; IP/UA gonderiliyor, TAMAMI `fbp` (riza kaniti) kapisinin arkasinda. **4,4 bu kalemden DEGIL:** `em`/`ph` hash'i BILEREK gonderilmiyor (gizlilik karari) — skoru yukseltmek OKAN/BaBa kapisi. GERCEK acik kalem **ATIF**: kampanya atfini `fbc` tasir, o da ayni kapinin arkasinda; ayrica toplama tarafi (index.html `PRUVO_ATIF` fbclid) riza kontrolsuz yaziyor -> **toplama ACIK, cikis KAPALI**, atfin nerede oldugu OLCULMEDI. KABUL (civilendi): D1 `siparis` son 30 gun SAYIM (deger okunmaz) — toplam · `atif` dolu · `fbp` · **`fbc`** · `ga_client_id` · `utm`. `fbc`≈0 ise kok neden riza kapisi (urun karari); `fbc` VAR ama Meta'da atif 0 ise kusur CAPI govdesinde ve KraL'in isi. OLCULEMEDI: sayim, **baglam kotasi RED'ine** takildi (tur=439/500, baglam=463K/450K).
