...
🔴 **K407 AÇILDI — SINIF: CI dışı kabul şartı yorumda yaşarsa koruma değildir.** `tools/mimar-kapi-mutasyon-test.py` son satırında `SONUC: KIRMIZI` (39 mutasyon eşiği tutmuyor) + `TABAN KIRMIZI: 92` basarken **çıkış kodu 0** döndürüyor ⇒ koşulsa bile YEŞİL sanılır. Kardeşi `mimar-kilit-test.py` **231/323 (92 ❌)**, düşen küme aynı. `.github/workflows/*.yml`'de ikisine de TAM AD atıf **0** — `ci-kapsam-test.py:2808` bunu bilerek yapmış ve riski yazmış: *"kapiyi degistiren her is bu iki bataryayi kendi agacinda kosturmak ZORUNDADIR (kabul sarti)"*. 11 Eyl'de üç kapı REDDETMEYİ bırakınca (Okan emri) o şart koşulmadı, kırmızı sessizce birikti. Aynı bayatlık 3. yerde de var (`kapi-envanteri.py`: "reddetmesi gerekeni REDDETMEDI") ⇒ **3. tekrar, tekil yama YASAK**. Çip `task_2b98be37`. Ders hafızada: `ci-disi-kabul-sarti-yorumda-yasarsa-koruma-degildir`.
🔧 **K406② ÖNCÜLÜ YERELDE ÇÜRÜDÜ:** "üç `--kendini-test` kolu tüm vakaları geçerken rc=2" iddiası bugün yerelde tutmadı — `devam-sinif-kapisi.py --kendini-test` (87 kontrol · 0 hata) **rc=0**, `ic-rapor-adi-kapisi.py --kendini-test` (22/22) **rc=0**; `deploy-aclik-kapisi.py` ÖLÇÜLMEDİ. Gerçek hijyen-a3 kırmızısı CI logundan okunacak (aynı çipte, bacak A).
